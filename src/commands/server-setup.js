...
async function getOrCreateChannel(guild, category, channelName) {
  ...
  if (!channel) {
    channel = await guild.channels.create({ name: channelName, type: 0, parent: category.id });
...
async function getOrCreateRole(guild, roleName, roleColor) {
  ...
  if (!role) {
    role = await guild.roles.create({ name: roleName, color: roleColor, reason: `Role for users with ${roleName}` });
  }
  return role;
}
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      if (!config.lastUpdated) config.lastUpdated = new Date().toISOString();
      const category = await getOrCreateCategory(guild, 'Engagement');

...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const fetchCount = Math.min(limit * 2, 50);
    const leaderboardItems = await queryLeaderboard(leaderboardType, guild.id, fetchCount);
    if (!leaderboardItems || leaderboardItems.length === 0) {
      return interaction.editReply({ content: `:x: No data found for the **${leaderboardType}** leaderboard in this server.` });
    }
    const validUsers = [];
    for (const item of leaderboardItems) {
      if (validUsers.length >= limit) break;
      try {
        const member = await guild.members.fetch(item.discordAccountId);
        validUsers.push({
    ...
    if (validUsers.length === 0) {
      return interaction.editReply({ content: `:x: No current members found with data for the **${leaderboardType}** leaderboard.` });
    }
    const canvafyUsers = validUsers.map((user, index) => ({
      top: index + 1,
      avatar: user.avatar,
      tag: user.username,
    ...
    const leaderboardTitle = leaderboardType.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
